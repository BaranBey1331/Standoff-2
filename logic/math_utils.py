...
def world_to_screen(pos, view_matrix, screen_w, screen_h):
    """
    3D koordinatları 2D ekran koordinatlarına çevirir (World-to-Screen).
    view_matrix: 4x4 matris (16 değer)
    """
    # Matris çarpımı (Basitleştirilmiş temsil)
    ...
    # NDC (Normalized Device Coordinates) dönüşümü
    ndc_x = clip_x / clip_w
    ndc_y = clip_y / clip_w
...
def calculate_aim_angles(local_pos, enemy_pos):
    """
    İki nokta arasındaki Yaw (Yatay) ve Pitch (Dikey) açılarını hesaplar.
    """
    ...
    # Yaw: Yatay düzlemdeki açı
    yaw = math.atan2(dy, dx) * 180 / math.pi

    # Pitch: Dikey düzlemdeki açı
...
def smooth_angle(current_angle, target_angle, smooth_factor):
    """
    Aimbot'un daha insansı hareket etmesi için yumuşatma uygular.
    """
